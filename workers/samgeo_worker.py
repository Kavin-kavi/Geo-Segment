...
def _ensure_geoai_attributes() -> None:
    """Patch missing geoai top-level attributes needed by samgeo.

    samgeo's save_masks may call geoai.raster_to_vector internally.
    If geoai's __init__.py was patched for DLL-guard mode (QGIS Windows)
    and the torch-dependent geoai.geoai sub-module failed silently,
    raster_to_vector is absent from the geoai namespace.  We import it
    directly from geoai.utils.raster and attach it so samgeo can proceed.
    """
    try:
        import geoai

        if not hasattr(geoai, "raster_to_vector"):
            try:
                from geoai.utils.raster import raster_to_vector

                geoai.raster_to_vector = raster_to_vector
            except Exception as exc:
                # Cannot patch — save_masks will raise AttributeError later
                # with the original samgeo error message.
                print(
                    f"[samgeo_worker] could not patch geoai.raster_to_vector: {exc}",
                    file=sys.stderr,
                )
    except Exception:
        pass
