"""Virtual environment manager for Geo Segment QGIS Plugin.

Manages the creation, package installation, and verification of a
virtual environment used to isolate Geo Segment's Python dependencies
...
CACHE_DIR = (
    os.environ.get("GEO_SEGMENT_CACHE_DIR")
    or os.environ.get("GEO_SEGMENT_VENV_DIR")
    or os.path.expanduser("~/.qgis_geo_segment")
)
VENV_DIR = os.path.join(CACHE_DIR, f"venv_{PYTHON_VERSION}")

REQUIRED_PACKAGES = [
    # geoai.utils Python code uses numpy.lib.array_utils which was introduced
    # in NumPy 2.0.  Do not pin to 1.x — let the resolver pick NumPy 2.x.
    # The "compiled using NumPy 1.x" error happens when geoai is imported
    # inside the QGIS process (which may have an older numpy); the fix is to
    # always run geoai code in the venv subprocess, never in the QGIS process.
    ("numpy", ""),  # unconstrained — let pip/uv choose compatible 2.x
    ("torch", ">=2.0.0"),
    ("torchvision", ">=0.15.0"),
    ("geoai-py", ""),
    ("segment-geospatial", ""),
...
_INSTALL_LOGIC_VERSION = "12"  # numpy unconstrained (geoai needs 2.x)
...
def _log(message: str, level=Qgis.Info):
    ...
    """
    QgsMessageLog.logMessage(message, "Geo Segment", level=level)
...
def _log_system_info():
    ...
    if os.environ.get("GEO_SEGMENT_CACHE_DIR"):
        info_lines.append(f"  GEO_SEGMENT_CACHE_DIR: {os.environ['GEO_SEGMENT_CACHE_DIR']}")
    elif os.environ.get("GEO_SEGMENT_VENV_DIR"):
        info_lines.append(f"  GEO_SEGMENT_VENV_DIR: {os.environ['GEO_SEGMENT_VENV_DIR']}")
...
def install_dependencies(
    ...
    # -- Partition packages into CUDA (individual), numpy (pre-install), and batch groups -----------
    cuda_packages: List[Tuple[str, str]] = []
    numpy_package: Optional[Tuple[str, str]] = None
    ...
    for pkg_name, ver_spec in required_packages:
        if cuda_enabled and pkg_name in ("torch", "torchvision"):
            ...
            # Separate numpy to install first before other packages that depend on it
            numpy_package = (pkg_name, ver_spec)
        else:
            batch_packages.append((pkg_name, ver_spec))

    # Progress allocation: numpy pre-install, CUDA phase, then batch phase
    numpy_progress_weight = 5 if numpy_package else 0
    cuda_progress_weight = 40 if cuda_packages else 0
    numpy_start = base_progress
    numpy_end = base_progress + int(progress_range * numpy_progress_weight / 100)
    cuda_start = numpy_end
    cuda_end = numpy_end + int(progress_range * cuda_progress_weight / 100)
    ...
    # -- Phase A-2: Pre-install numpy BEFORE other packages ----
    # This ensures geoai-py and other packages that depend on numpy
    # are built/installed against the correct numpy version.
    if numpy_package:
        pkg_name, ver_spec = numpy_package
        package_spec = f"{pkg_name}{ver_spec}"
        
        if progress_callback:
            progress_callback(numpy_start, f"Installing {pkg_name}...")
        
        _log(
            f"[Pre-install] Installing {package_spec} first (before geoai-py and others)...",
            Qgis.Info,
        )
        ...
        try:
            result = _run_pip_install(
                ...
                env=env,
                subprocess_kwargs=subprocess_kwargs,
                label=f"numpy (pre-install for geoai-py compatibility)",
                progress_start=numpy_start,
                progress_end=numpy_end,
                progress_callback=progress_callback,
                cancel_check=cancel_check,
            )
            ...
            if result.returncode != 0:
                ...
                _log(
                    f"Failed to pre-install numpy: {error_msg[:500]}",
                    Qgis.Critical,
                )
                if _is_ssl_error(error_msg):
                    return False, "Failed to install numpy: SSL certificate error"
                if _is_proxy_auth_error(error_msg):
                    return False, "Failed to install numpy: proxy authentication required"
                if _is_network_error(error_msg):
                    return False, "Failed to install numpy: network error"
                return False, f"Failed to install numpy: {error_msg[:200]}"
            
            _log(
                f"Successfully pre-installed {package_spec}",
                Qgis.Success,
            )
            if progress_callback:
                progress_callback(numpy_end, f"{pkg_name} installed")

        except subprocess.TimeoutExpired:
            _log(f"Installation of {package_spec} timed out", Qgis.Critical)
            return False, f"Installation of {package_spec} timed out"
        except Exception as e:
            _log(f"Exception pre-installing {package_spec}: {e}", Qgis.Critical)
            return False, f"Error pre-installing {package_spec}: {str(e)[:200]}"
...
def _get_verification_code(package_name: str) -> str:
    ...
    if package_name == "torch":
        ...
        # geoai uses numpy.lib.array_utils which requires numpy>=2.0.
        return (
            "import numpy as _np; "
            "_v = _np.__version__; "
            "from packaging.version import parse as _pv; "
            "assert _pv(_v) >= _pv('2.0'), "
            "f'numpy>=2.0 required for geoai (got {_v})'; "
            "print(_v)"
        )
    elif package_name == "geoai-py":
        return (
            "import geoai; print(geoai.__version__); "
            "from geoai.utils.raster import raster_to_vector; "
            "print('geoai.utils OK')"
        )
