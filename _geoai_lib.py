...
def get_geoai() -> ModuleType:
    ...
    raise ImportError(
        "Geo Segment plugin could not import the external 'geoai' library.\n\n"
        f"QGIS Python:\n  executable: {py}\n  version: {ver}\n"
        f"{venv_check}\n\n"
        f"Strategy diagnostics:\n{diag_text}\n\n"
        "Fix: Click any Geo Segment toolbar button to open the dependency installer,\n"
        "which will automatically download and install all required packages.\n\n"
        "Alternatively, install the geoai-py Python package manually:\n"
